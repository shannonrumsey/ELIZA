...
class Eliza:
    ...
    def _next_reasmb(self, decomp):
        if self.indices == []:
            self.indices = random.sample(range(len(decomp.reasmbs)), len(decomp.reasmbs))
    ...
    def respond(self, text):
        ...
        output = None
        exceptions = [["i", "am"], ["i", "am", "not"]]
        if words in exceptions:
            output = self._next_reasmb(self.keys['declarative'].decomps[0])
            log.debug('Output from declarative: %s', output)
        else:
            for key in keys:
